...
Los ADR **0001–0012** son las decisiones estructurales del arquitecto. Los **0013–0023** consolidan los ADR propuestos en `11-privacidad-y-voto-secreto.md` (ADR-110 a ADR-125), deduplicados. Los **0024–0042** elevan decisiones arquitectónicas que estaban dispersas en el resto del corpus. Los **0043–0045** fijan los primeros incrementos de ejecución posteriores al corte vertical auditado. Los **0046–0048** cierran la deliberación estructurada, los métodos de escrutinio que faltaban y el análisis de consenso transversal. El **0049** sustituye parcialmente al 0046: retira el sellado criptográfico de la autoría y lo reemplaza por una regla de acceso con alcance de etapa. El **0050** revisa el umbral de no-facción del 0048 y no manda hasta que se acepte: es **Propuesto**, no aceptado —pero no es el primero de la tabla en ese estado; ver la nota de recuento más abajo. El **0051** implementa por fin la sección 6 de `GOVERNANCE.md`: las reglas de gobernanza como datos versionados que el administrador técnico no puede escribir. El **0052** cierra el principio 6 —«la IA asesora, nunca gobierna»— sacándolo de la prosa: el `AIAssistantPort` que `ARCHITECTURE.md` §6 llevaba declarado y sin implementar tiene ahora un tipo de retorno que **no puede** expresar una decisión, y las 27 preguntas del formulario funcionan sin ningún proveedor de IA. El **0053** cierra el ciclo por el otro extremo: la evaluación contra criterios que no se pueden mover, el resultado como dato derivado que se recomputa al leer y una memoria de aprendizajes sin autor, construida para que una evaluación **pueda salir mal** sin señalar a nadie. El **0054** es el más reciente de los ADR **Propuesto** del registro y el que menos decide a propósito: registra que el **padrón** —el denominador de todas las reglas de decisión— es el único estado de gobierno **fuera** del historial encadenado, describe el ataque que eso habilita y por qué ni el congelado del 0025 ni el verificador independiente lo ven, y deja planteada la elección de fondo, que **no le toca a quien escribe código**: si la supresión pasa a significar «destrucción irreversible del vínculo» —conservando un seudónimo huérfano, que es lo que el 0008, el 0009 y el 0021 ya sostienen de facto— o si se conserva el borrado absoluto y se **declara** que la autenticidad del padrón depende de una autoridad externa cofirmante y no del registro. Recomienda lo primero, con el riesgo jurídico escrito y tres preguntas para un abogado.

> **Corregido el 2026-08-24 — este párrafo se contradecía con su propia tabla.** Decía que el 0050
> era «el primer ADR Propuesto del registro» y el 0054 «el segundo», pero la tabla de abajo ya listaba
> **0018 y 0019** (Belenios, custodios 3-de-5) y **0042** (ESAL como Responsable) como `Propuesto`
> desde antes de que existieran el 0050 y el 0054. El recuento verificado contando las filas de la
> tabla (`grep -c` sobre este fichero, 2026-08-24): **49 Aceptado, 5 Propuesto** (0018, 0019, 0042,
> 0050, 0054), **0 Depreciado**, sobre 54 ADR numerados. El 0050 y el 0054 son, cada uno, el más
> reciente en llegar a ese estado — no el primero ni el segundo del registro.
...
| [0054](0054-procedencia-del-padron.md) | Procedencia del padrón | **Propuesto** | El censo es el denominador de **todas** las reglas de §4 y el **único estado de gobierno que no está en el historial encadenado**: vive en `identity.member`, tabla mutable, el alta no emite evento y `registryVersion` es la constante `1`. Congelar (0025) desplaza la ventana pero conserva para siempre lo que se congele mal, y el verificador **certificaría una mentira coherente**. La reparación —padrón por eventos, sólo con el `MemberId` de 128 bits que 0006/0007 permiten— **es incompatible con el borrado absoluto de la pertenencia pasada**, y hay que elegir: **(A)** supresión = destrucción irreversible del vínculo, conservando el seudónimo huérfano (lo que 0008/0009/0021 ya sostienen), o **(B)** borrado absoluto y autenticidad del padrón **declarada** externa. **Recomienda (A) y NO decide:** la elección es jurídica y política. |
