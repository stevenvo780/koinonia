...
pie mientras nadie mirara, que es justo lo contrario de lo que el ADR-0026 pide. Ver la errata **E94**.
...
- **`cumplido` / `incumplido` sigue siendo un juicio humano.** El criterio congelado trae `description`
  y `evidenceSource`, pero **no un umbral**: no hay un `successIf` que comparar (ver **E90**). La
...
- **`Agreement` sigue sin existir como agregado** (ver **E90**). Lo que se evalúa es el plan de
...
El registro venía por **E87** (ADR-0052); se empieza en **E90** para no chocar con otro incremento en
curso. **E88 y E89 quedan reservados**, y esta numeración no implica que existan.
