...
| Estado | Requisito |
| --- | --- |
| `PARCIAL` | Se deben poder elegir los métodos desde la interfaz o desde la API al abrir una decisión. `apps/web/app/decisiones/abrir/page.tsx` lista los 9 métodos del catálogo con radios reales, y el contrato `emitirPapeleta` ya transporta las seis clases de papeleta que el motor exige —puntuación, orden y menciones incluidas—, así que los cuatro métodos que antes no tenían por dónde responder ya se pueden votar de punta a punta (`tests/integration/http-metodos.test.ts`). **Falta**: habilitar en el servidor la apertura de decisiones sobre varias opciones — hoy una votación se abre sobre un solo texto — para que esos 4 métodos, que EXISTEN para comparar varias salidas, dejen de estar deshabilitados en la propia pantalla de apertura (`apps/web/app/decisiones/abrir/page.tsx`, `sePuedeAbrirHoy`); hasta entonces se pueden abrir por API directa, sin la garantía de producto de que la comparación diga algo. |
| `PARCIAL` | Implementar doce métodos de votación: mayoría simple, supermayoría, consentimiento, consenso, advice process, score voting, ranked choice, majority judgment y los demás. `ID_METODOS` (`packages/contracts/src/metodos.ts:47-56`) tiene exactamente **9, no 12**. **Falta**: los 3 métodos restantes del pliego original de doce (o una justificación documentada de por qué se reduce a 9) — y de los 9 existentes, sólo 5 son abribles hoy desde la interfaz (los otros 4 ya se pueden responder si existen; lo que falta es poder abrirlos, ver la fila de arriba). |
