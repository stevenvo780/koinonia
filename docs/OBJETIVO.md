...
| `CUMPLE` | Implementar doce métodos de votación: mayoría simple, supermayoría, consentimiento, consenso, advice process, score voting, ranked choice, majority judgment y los demás. `ID_METODOS` tiene **11**, y los OCHO nombrados en el pliego están todos: mayoría simple, supermayoría, consentimiento, **consenso**, **proceso de consejo**, puntuación, orden de preferencia y menciones. Los tres restantes hasta doce son «y los demás» del propio pliego: unanimidad, comparación por pares y deliberación por sorteo. Los dos últimos se construyeron el 2026-08-30, enteros —motor, papeleta propia, codec, ruta, pantalla y cierre— y **los dos se pueden abrir**, que es lo que los distingue de los cuatro comparativos: con 6 abribles antes, ahora son **7**. El proceso de consejo decide sobre la única propuesta que hay; el consenso también, y añade la figura que ninguno de los otros tenía: **apartarse** —«no lo apoyo, no lo voy a impedir, y quiero que conste»— con un tope de cuántos pueden hacerlo antes de que el acuerdo deje de significar algo. Ésa es la distinción que el motor hace cumplir distinto de unanimidad y de consentimiento, y es la que faltaba para que «consenso» fuera un método y no una etiqueta. Comprobados de punta a punta contra PostgreSQL en `tests/integration/http-proceso-de-consejo.test.ts`, cada uno en el caso que lo define: quien decide resolviendo EN CONTRA de todos los consejos, y un acuerdo que nadie bloquea pero que no pasa porque se apartaron demasiados. |
