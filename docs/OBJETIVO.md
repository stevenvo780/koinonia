...
| --- | --- | --- | --- |
| 0 | **Verificar lo no verificado** | los 27 `NO VERIFICADO`, sobre todo las 16 amenazas y los 9 puntos de tecnología | en curso — 2026-08-23: T-06 y T-25 verificados (`PARCIAL`, ver tabla de seguridad-y-tecnologia por lo que falta de cada uno), T-12 verificado con un defecto real sin corregir, T-19 con guardia de dominio lista y probada pero sin cablear a la API. Faltan T-01–T-05, T-07–T-11, T-20 y los 9 puntos de TECNOLOGÍA. |
...
| 3 | **Pruebas** | matriz completa de navegadores **ejecutada**; exploratorias masivas; carga con k6; umbrales de cobertura; pipeline escalonado | pendiente |
...
| `PARCIAL` | T-12 (Spam/saturación deliberativa) — 3 propuestas/7d, 20 comentarios/24h, 1 objeción/proceso (2ª con respaldo). **Verificado 2026-08-23**: cupos de propuesta (3/7d) y comentario (20/día) implementados en `services/api/src/http/rate-limit.ts` y probados bajo carga concurrente real en `tests/integration/rate-limits.test.ts` — 10 peticiones simultáneas dieron exactamente 3×201+7×429, 30 dieron exactamente 20 aceptadas+10×429; el mecanismo es atómico (`INSERT … ON CONFLICT DO UPDATE … RETURNING`) y no deja pasar de más bajo carrera. **Defecto real encontrado y NO corregido**: en `POST /propuestas` y `POST /deliberaciones/:id/aportes`, el cupo se consume (`cupoDePropuesta`/`cupoDeComentario`) **antes** de que la ruta detecte un `requestId` repetido; un reintento de idempotencia normal (el patrón que el propio sistema exige para reintentos seguros «móvil primero») gasta un cupo real sin crear nada. Objeción por proceso (1, 2ª con respaldo) sigue sin verificar. |
...
| `NO CUMPLE` | Coverage thresholds (líneas, ramas, funciones, sentencias) por paquete; fallar build si debajo. |
...
| `PARCIAL` | Matriz de navegadores: Chromium, Firefox, WebKit (escritorio); Chrome móvil, Safari móvil; responsive. |
...
| `PARCIAL` | Pipeline escalonado: pre-commit → PR (Chromium) → main (matriz completa) → nightly (exploratorias, carga, mutation). |
