...
| `PARCIAL` | Implementar doce métodos de votación: mayoría simple, supermayoría, consentimiento, consenso, advice process, score voting, ranked choice, majority judgment y los demás. `ID_METODOS` (`packages/contracts/src/metodos.ts:47-56`) tiene **9**. El pliego nombra ocho y cierra con «y los demás»; de los nombrados están seis —mayoría simple, supermayoría, consentimiento, puntuación, orden de preferencia y menciones— y los otros tres del catálogo (unanimidad, comparación por pares, deliberación por sorteo) son «los demás». **Faltan dos nombrados, y acá va la justificación documentada que esta misma fila admitía como salida:** (a) **advice process** no es un método de escrutinio: es «una persona decide después de consultar a quien le toca», y exige lo que el producto no tiene —un papel de «quien decide» por asunto y una compuerta que compruebe que la consulta ocurrió—. Contarlo como un noveno «método de votación» sería llamar votación a algo que no lo es, y abrirlo sin esas dos piezas produciría un resultado que no significa nada. (b) **consenso**, como categoría distinta, se solapa con dos que ya existen y que son los dos extremos coherentes: `unanimity` —todo el mundo a favor— y `sociocratic-consent` —nadie objeta con daño argumentado—. Un tercer punto intermedio sería una distinción que el motor no podría hacer cumplir de forma distinta a esas dos, o sea un nombre más en la lista y nada más. **Lo honesto es 9 con esta nota, no 12 con dos de relleno**; si la asamblea quiere el advice process de verdad, lo que hay que construir primero son el papel de quien decide y la compuerta de consulta, no un `tally` nuevo — y de los 9 existentes, sólo 5 son abribles hoy desde la interfaz (los otros 4 tienen papeleta pero no se pueden abrir, ni por pantalla ni por API, hasta que una decisión pueda llevar más de una opción; ver la fila de arriba). |
